    ...
    <div className="p-8 space-y-6">
      ...
      <div className="rounded-3xl overflow-hidden" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
        {loading ? (
          ...
          <div className="p-12 text-center text-sm" style={{ color: "rgba(148,163,184,0.5)" }}>
            No messages yet.
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.02)" }}>
                  <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-[0.08em]" style={{ color: "rgba(148,163,184,0.5)" }}>
                    Status
                  </th>
                  <th className="px-6 py-4 text-left text-[11px] font-bold uppercase tracking-[0.08em]" style={{ color: "rgba(148,163,184,0.5)" }}>
                    Topic
                ...
                {messages.map((msg) => (
                  <tr
                    ...
                    style={{
                      ...
                      background: msg.isRead ? "transparent" : "rgba(124,58,237,0.06)",
                    }}
                  >
                    ...
                    <td className="px-6 py-4 font-semibold text-slate-200">{msg.subject}</td>
                    <td className="px-6 py-4 text-xs font-medium whitespace-nowrap" style={{ color: "rgba(148,163,184,0.5)" }}>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 p-4 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)" }}>
          <div className="w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl" style={{ background: "rgba(15,15,25,0.98)", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div className="p-6 space-y-4 text-sm">
              <div>
                <p className="text-xs font-semibold mb-1" style={{ color: "rgba(148,163,184,0.5)" }}>From</p>
                <p className="font-bold text-slate-100">
                  {selected.name} <span className="text-slate-400 font-normal">({selected.email})</span>
                </p>
              </div>
              <div>
                <p className="text-xs font-semibold mb-1" style={{ color: "rgba(148,163,184,0.5)" }}>Topic</p>
                <p className="font-semibold text-violet-400">{TOPIC_LABEL[selected.topic]}</p>
              </div>
              <div>
                <p className="text-xs font-semibold mb-1" style={{ color: "rgba(148,163,184,0.5)" }}>Subject</p>
                <p className="font-bold text-slate-100">{selected.subject}</p>
              </div>
              <div>
                <p className="text-xs font-semibold mb-1" style={{ color: "rgba(148,163,184,0.5)" }}>Message</p>
                <div className="p-4 rounded-2xl leading-relaxed text-slate-300" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  {selected.message}
                </div>
              </div>
