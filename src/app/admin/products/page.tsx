...
const BADGE_STYLE: Record<string, string> = {
  ...
  "Best Seller": "bg-amber-900/30 text-amber-400 border border-amber-500/30 whitespace-nowrap",
...
function ProductModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm  p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
      <div className="rounded-3xl shadow-2xl  w-full max-w-lg flex flex-col max-h-[92vh]" style={{ background: "rgba(15,15,25,0.98)", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="flex items-center justify-between px-7 py-5 border-b border-white/5">
          <div>
            <h2 className="text-lg font-bold text-slate-100">{product ? "Edit Product" : "New Product"}</h2>
            <p className="text-xs text-slate-500 mt-0.5">{product ? "Update product details" : "Add a product to your store"}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl text-slate-500 hover:text-slate-300 hover:bg-white/[0.04] transition-colors">
        ...
        <form id="product-form" onSubmit={submit} className="flex-1 overflow-y-auto px-7 py-5">
          <div className="space-y-4">
            <Field label="Product Name *">
              <input name="name" required value={form.name} onChange={set} placeholder="e.g. Classic Linen Shirt" className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60" />
            </Field>
            <Field label="Description *">
              <textarea name="description" required rows={3} value={form.description} onChange={set} className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60 resize-none" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Price ($) *">
                <input name="price" type="number" min="0" step="0.01" required value={form.price} onChange={set} className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60" />
              </Field>
              <Field label="Original Price ($)">
                <input name="originalPrice" type="number" min="0" step="0.01" value={form.originalPrice} onChange={set} placeholder="Leave blank if none" className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60 placeholder-gray-300" />
              </Field>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Sizes">
                <input name="sizes" value={form.sizes} onChange={set} placeholder="S, M, L, XL" className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60 placeholder-gray-300" />
              </Field>
              <Field label="Colors">
                <input name="colors" value={form.colors} onChange={set} placeholder="Black, White" className="px-3.5 py-3 rounded-xl text-sm bg-white/[0.05] border border-white/10 text-slate-100 focus:outline-none focus:border-violet-500/60 placeholder-gray-300" />
              </Field>
            </div>
            ...
            <Field label="Stock Quantity">
              ...
              />
              <p className="text-[11px] text-slate-500 mt-1">How many units are currently available</p>
            </Field>
      ...
      <div className="rounded-2xl  overflow-hidden" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <tr key={p._id} className={`hover:bg-violet-50/20 transition-colors duration-100 group ${!p.isVisible ? "opacity-60" : ""}`}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-100 max-w-[180px] truncate">{p.name}</p>
                    ...
                    <td className="px-5 py-4 whitespace-nowrap">
                      <span className="text-sm font-black text-slate-100">৳{p.price}</span>
                      {p.originalPrice && (
                        <span className="text-xs text-slate-500 line-through ml-1.5">৳{p.originalPrice}</span>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      {p.badge ? (
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${BADGE_STYLE[p.badge] ?? "bg-white/[0.06] text-slate-300"}`}>
                          {p.badge}
                        </span>
                    ...
                    <td className="px-5 py-4">
                      {(() => {
                        const qty = p.stock ?? 0;
                        if (!p.inStock) return <span className="text-xs font-semibold text-red-400">Out of stock</span>;
                        if (qty === 0) return <span className="text-xs text-slate-500">—</span>;
                        const color = qty <= 3 ? "text-red-400 font-bold" : qty <= 10 ? "text-amber-400 font-semibold" : "text-emerald-400 font-semibold";
                    ...
                    <td className="px-5 py-4">
                      {p.isFeatured ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: "rgba(251,191,36,0.12)", color: "#fbbf24", border: "1px solid rgba(251,191,36,0.2)" }}>
        ...
        {pagination.pages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 bg-white/[0.01]">
            <p className="text-sm text-slate-500">Page <span className="font-bold text-slate-300">{page}</span> of {pagination.pages}</p>
            <div className="flex gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                className="px-4 py-2 text-sm font-semibold border border-white/8 rounded-xl hover:bg-white/[0.04] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">â† Prev</button>
              <button onClick={() => setPage((p) => Math.min(pagination.pages, p + 1))} disabled={page === pagination.pages}
                className="px-4 py-2 text-sm font-semibold border border-white/8 rounded-xl hover:bg-white/[0.04] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Next â†’</button>
            </div>
          </div>
        )}
      </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm  p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="rounded-3xl shadow-2xl  w-full max-w-sm p-8" style={{ background: "rgba(15,15,25,0.98)", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <h3 className="text-xl font-bold text-slate-100 mb-2">Delete Product</h3>
            <p className="text-sm text-slate-500 mb-7">This will permanently remove the product from your store.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)}
                className="flex-1 px-5 py-3 border border-white/8 rounded-2xl text-sm font-semibold text-slate-300 hover:bg-white/[0.02]">Cancel</button>
