      ...
      .then((data) => {
        if (data?.data) {
          ...
          setShippingInfo((prev) => ({
            ...prev,
            firstName: defaultAddr?.firstName || user.name?.split(" ")[0] || prev.firstName,
            lastName: defaultAddr?.lastName || user.name?.split(" ").slice(1).join(" ") || prev.lastName,
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      <label className="block text-xs font-medium text-charcoal-600 mb-2">Division <span className="text-red-500">*</span></label>
                      <select
                        className={`input-field bg-white ${shippingErrors.city ? "border-red-400 focus:ring-red-200 focus:border-red-400" : ""}`}
                        value={shippingInfo.city}
                        onChange={(e) => {
                          const newDiv = e.target.value;
                          setShippingInfo({...shippingInfo, city: newDiv, state: "", zip: ""});
                          setShippingErrors({...shippingErrors, city: "", state: "", zip: ""});
                        }}
                      >
                    ...
                    {/* District */}
                    <div className="col-span-2 sm:col-span-1">
                      <label className="block text-xs font-medium text-charcoal-600 mb-2">District <span className="text-red-500">*</span></label>
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setShippingInfo({...shippingInfo, state: newDist, zip: ""});
                          setShippingErrors({...shippingErrors, state: "", zip: ""});
                        }}
                      >
                        <option value="">{!shippingInfo.city ? "Select Division First" : "Select District"}</option>
                    ...
                    {/* Thana */}
                    <div className="col-span-2">
                      <label className="block text-xs font-medium text-charcoal-600 mb-2">Thana <span className="text-red-500">*</span></label>
                      <select
                      ...
                      >
                        <option value="">{!shippingInfo.state ? "Select District First" : "Select Thana"}</option>
                        {getThanas(shippingInfo.city, shippingInfo.state).map((thana) => (
                    ...
                    {[
                      ...
                      return (
                        <button
                          key={m.id}
                          type="button"
                          onClick={() => { setPaymentInfo({ method: m.id as any, txnId: "" }); setPaymentErrors({}); }}
                          className={`relative flex flex-col items-start justify-between gap-3 px-4 py-4 rounded-2xl border-2 transition-all duration-200 text-left min-h-[92px] ${
                            active ? "border-charcoal-950 shadow-md bg-warm-50/20" : "border-charcoal-200 hover:border-charcoal-400 bg-white"
                          }`}
                        >
                          {active && (
                          ...
                          {m.logo ? (
                            ...
                            <img src={m.logo} alt={m.alt} className="h-7 max-w-[120px] object-contain shrink-0" />
                          ) : (
                  ...
                  <div className="space-y-6">
                    <div className="bg-warm-50 rounded-xl p-5 border border-warm-100">
                      ...
                      <p className="text-sm text-charcoal-500 font-light leading-relaxed">
                        {shippingInfo.firstName} {shippingInfo.lastName} &bull; {shippingInfo.phone}<br />
                        {shippingInfo.address && <>{shippingInfo.address}<br /></>}
                        {[shippingInfo.zip && `Thana: ${shippingInfo.zip}`, shippingInfo.state && `District: ${shippingInfo.state}`, shippingInfo.city && `Division: ${shippingInfo.city}`].filter(Boolean).join(" • ")}
                      </p>
                    </div>
